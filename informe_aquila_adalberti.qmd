...
```{r  label = fig-fav, echo = FALSE, fig.cap = "Resultados de las correlaciones de Pearson de la favorabilidad de cada paso de la regresión con el resultado final de favorabilidad del águila imperial", out.width = '100%', fig.align='center'}
...
# ![Partición de la varianza entre factores"](%60r%20file.path(paths$figure_path,%22variance_plot_color.png%22)%60)
```
...
Para modelar la favorabilidad del Aguila imperial en Extremadura se realizó una preselección de las variables predictivas a partir del análisis de la correlación entre variables y la técnica FDR, descartándose x de las y variables incluidas inicialmente. Se ha configurado finalmente un modelo en `r future_n_steps` pasos con x variables explicativas estadísticamente significativas (Tabla 31). Las dos variables seleccionadas por el modelo (isotermalidad y la precipitación del mes más árido) han presentado signo negativo. Además, no se han encontrado en el modelo diferencias significativas (HyL \> 0,05) entre lo observado y lo esperado en la prueba de Hosmer y Lemeshow (HyL = `r future_blr$chisq_stat`).
...
historical_cat_2 + historical_cat_3
...
```{r  label = fig-future, echo = FALSE, fig.cap = "Partición de la varianza entre factores", out.width = '100%'}
knitr::include_graphics(file.path(paths$figure_path,"future_prediction_2.png"))

# ![Partición de la varianza entre factores\"](%60r%20file.path(paths$figure_path,%22future_prediction_2.png%22)%60){fig-alt="Future prediction" fig-align="center"}{#fig-variance}
```
